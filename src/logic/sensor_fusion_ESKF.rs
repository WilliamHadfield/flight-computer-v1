...
use crate::I2C::gps::GPS_CHANNEL_ESKF;
use crate::I2C::gps::GPS_VEL_CHANNEL_ESKF;
use crate::I2C::mpu6050::ImuData;
use crate::I2C::bmp390::BaroData;
use crate::SPI::lis3mdl::MagnoData;
use libm::powf;
use embassy_time::{Delay,Duration, Instant, Timer};
use crate::I2C::mpu6050::MPU_CHANNEL_EKF;
use crate::I2C::bmp390::BARO_CHANNEL_EKF;
use crate::SPI::lis3mdl::MAG_CHANNEL_EKF;
...
pub static ESKF_CHANNEL : Channel<ThreadModeRawMutex, ESKFState, 8> = Channel::new();
...
impl From<BaroData> for BaroMeasVec {
    fn from(d : BaroData) -> Self {
       let altitude = 44330.0 * (1.0 - powf(d.pressure / 101325.0_f32, 0.1903));
       SVector::from([
          altitude
       ])
   }
...
pub type SoftIronMat = SMatrix<f32,M_MAG, M_MAG>;
pub type HardIronVec = SVector<f32, M_MAG>;


impl From<MagnoData> for MagMeasVec {
   fn from(d : MagnoData) -> Self {
   let mut soft_iron: SoftIronMat = SoftIronMat::zeros();
   soft_iron[(0,0)] = 1.05333; soft_iron[(0,1)] = 0.00298449; soft_iron[(0,2)] = -0.0631712;
    soft_iron[(1,0)] = 0.00298449;  soft_iron[(1,1)] = 0.977784;  soft_iron[(1,2)] = 0.000794802;
     soft_iron[(2,0)] =  -0.0631712;  soft_iron[(2,1)] =  0.000794802; soft_iron[(2,2)] = 0.98015;
   
   let mut hard_iron: HardIronVec = HardIronVec::zeros();
   hard_iron[(0,0)] =  -0.29977;   hard_iron[(1,0)] =  -1.25668; 
    hard_iron[(2, 0)] =  0.968914; 
   
   
    let raw = SVector::<f32, 3>::from([d.x_gauss, d.y_gauss, d.z_gauss]);
    let calibrated = soft_iron * (raw - hard_iron);
    SVector::from(calibrated)
    }
}
...
// this turns them from radians per second to just radians
   ...
   let angle = sqrt(angle_old as f64) as f32;
    ...
    let (dq0, dq1, dq2, dq3) = if angle > 1e-6 {
        let s = sin((angle * 0.5 ) as f64) as f32 / angle;
        (cos((angle * 0.5) as f64) as f32, wx_dt * s, wy_dt * s, wz_dt * s)
    ...
    // renormalise
    let norm = sqrt((new_q0 * new_q0 + new_q1 * new_q1 + new_q2 * new_q2 + new_q3 * new_q3) as f64) as f32;
...
// matching orientation noise to oreination error measurment space to state space (error state sapce)
   ...
   // gyro bias noise to gyro bias error (measurment space to error space)
         ...
         // my process noise was calculated at a fix rate of 16 hz because of how i setup up the defmt info task for my allan deviation ouptut so to 
         // essentially correct this or make sure it doesnt have an adverse effect by multipliying the process noise by the timestep it essentially adapts the process noise to the timestep. 
        }
        ...
        pub fn update_baro(&mut self, baro : BaroMeasVec) {
         ...
         let dth_y = dx[7];
         let dth_z = dx[8];
         let dth_norm = sqrt((dth_x * dth_x + dth_y * dth_y + dth_z * dth_z)as f64) as f32;

         let (dq0, dq1, dq2, dq3) = if dth_norm > 1e-6 {
            let s_half = sin((dth_norm * 0.5)as f64) as f32 / dth_norm;
            (
              cos((dth_norm * 0.5)as f64) as f32,
              dth_x * s_half,
              dth_y * s_half,
              dth_z * s_half,
            )
         ...
         // renormalise
         let norm =
         sqrt((new_q0 * new_q0 + new_q1 * new_q1 + new_q2 * new_q2 + new_q3 * new_q3)as f64) as f32;
       ...
       let z_norm = sqrt((z[0] * z[0] + z[1] * z[1] + z[2] * z[2]) as f64) as f32;
       ...
       let m_norm = sqrt((m_world[0] * m_world[0] + m_world[1] * m_world[1] + m_world[2] * m_world[2]) as f64) as f32;
       ...
       let y02 = y[1];
       let y03 = y[2];
        let z_mag = sqrt((z[0] * z[0] + z[1] * z[1] + z[2] * z[2]) as f64) as f32;
        defmt::info!("z_mag : {}, y: {} {} {}", z_mag, y01, y02, y03);
...
// orientation error dtheta = dx[6..9] -> small quartenion q * dq (* = quartenion addition or the (+) symbol)
let dth_x = dx[6];
let dth_y = dx[7];
let dth_z = dx[8];
let dth_norm = sqrt((dth_x * dth_x + dth_y * dth_y + dth_z * dth_z)as f64) as f32;
...
let baro_rx = BARO_CHANNEL_EKF.receiver();
let mag_rx = MAG_CHANNEL_EKF.receiver();
let gps_v_rx = GPS_VEL_CHANNEL_ESKF.receiver();
let gps_p_rx = GPS_CHANNEL_ESKF.receiver();

let mut eskf = ESKF::new();

// logic to coast both gps channels
let mut pos_pending: Option<(f32, f32)> = None;
let mut vel_pending: Option<(f32, f32)> = None;
...
if let Ok(imu) = imu_rx.try_receive() {
    eskf.predict(imu.into(), dt);
}


if let Ok(baro) = baro_rx.try_receive() {
   eskf.update_baro(baro.into());
}

if let Ok(mag) = mag_rx.try_receive() {
  eskf.update_mag(mag.into());
}

// gps combine the two channels
if let Ok(pos) = gps_p_rx.try_receive() {
    defmt::info!("pos : {}", pos);
    pos_pending = Some(pos);
}

if let Ok(vel) = gps_v_rx.try_receive() {
    defmt::info!("vel : {}", vel);
        vel_pending = Some(vel);
}

if let (Some(p), Some(v)) = (pos_pending.take(), vel_pending.take()) {
    let mut gps_vec = GpsMeasVec::zeros();
    gps_vec[(0,0)] = p.0;
   gps_vec [(1,0)] = p.1;
   gps_vec [(2,0)] = v.0;
    gps_vec[(3,0)] = v.1;

    eskf.update_gps(gps_vec);
...
velocity_y : eskf.x[4],
velocity_z : eskf.x[5],
q_rotations_scalar : eskf.x[6], // i said this before but this should be close to one or equal to one
q_rotations_1 : eskf.x[7],
q_rotations_2 : eskf.x[8],
q_rotations_3 : eskf.x[9],
accel_bias_x : eskf.x[10],
accel_bias_y : eskf.x[11],
accel_bias_z : eskf.x[12],
gyro_bias_x : eskf.x[13],
gyro_bias_y : eskf.x[14],
gyro_bias_z : eskf.x[15],
};


  Timer::after_millis(15).await;
let _ = ESKF_CHANNEL.sender().try_send(state);
